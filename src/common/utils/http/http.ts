...
import { defaultEncode, decryption, encryption } from '../crypto/crypto';
import LocalStorageService from '../cache/local-storage';
...
const MD5 = ENVConfig.MD5;
const DES_KEY = ENVConfig.DES_KEY;
const DES_IV = ENVConfig.DES_IV;
...
const encode = (params: string) => {
    const signature = defaultEncode(params + MD5);
    return encryption(JSON.stringify({ params, signature }), DES_KEY, DES_IV);
};
...
class HttpService {
    ...
    public getLocalStorageInfo(): any {
        ...
        const decryptUserInfo = (userInfo && JSON.parse(decryption(userInfo as string, DES_KEY, DES_IV)).jsonData) || {};
        const token = decryptUserInfo.token || '';
        const userId = decryptUserInfo.id || null;
        const userName = decryptUserInfo.userName || '';
        const nickName = decryptUserInfo.nickName || '';

        return { userInfo, decryptUserInfo, token, userId, userName, nickName };
    }
    ...
    post(bodyParams: any, config: any) {
        ...
        const _bodyParams = (_config.des && {
            body: encode(JSON.stringify({ ...bodyParams, token, ...user }))
        }) || { ...bodyParams, token, ...user };
